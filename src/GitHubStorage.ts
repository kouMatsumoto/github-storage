...
export class GitHubStorage {
	...
	async defaultBranch() {
		...
		return this.#client.getRepositoryDefaultBranch({
			owner: username,
	...
	async findIndices({ count }: { count: number }): Promise<FileIndex[]> {
		...
		while (hasNextPage && results.length < count) {
			const result: Awaited<ReturnType<GitHubClient["getRepositoryCommits"]>> =
				await this.#client.getRepositoryCommits({
					owner: username,
					name: this.#repository,
	...
	async load({ count }: { count: number }): Promise<FileData[]> {
		const { username } = await this.#client.getViewer();
		const { defaultBranchName, commits } =
			await this.#client.getRepositoryCommits({
				owner: username,
				name: this.#repository,
				count,
		...
		const records = await Promise.all(
			commits.map(async ({ message }) => {
				...
				try {
					...
					console.error(
						`[github-storage] data is not json schema: ${file.text}`,
					);
					return undefined;
				}
			}),
	...
	async save({
		...
		const { username } = await this.#client.getViewer();
		const { defaultBranchName, lastCommitId } =
			await this.#client.getRepositoryCommits({
				owner: username,
				name: this.#repository,
				count: 1,
